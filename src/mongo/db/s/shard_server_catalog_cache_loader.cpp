...
DatabaseType getPersistedDbVersion(OperationContext* opCtx, StringData dbName) {
    ShardDatabaseType shardDatabaseEntry =
        uassertStatusOK(readShardDatabasesEntry(opCtx, dbName));

    DatabaseType dbt(shardDatabaseEntry.getDbName(), shardDatabaseEntry.getPrimary(), shardDatabaseEntry.getPartitioned(), shardDatabaseEntry.getDbVersion());
 ...
 */
void forcePrimaryRefreshDbVersionAndWaitForReplication(OperationContext* opCtx, StringData dbName) {
    auto const shardingState = ShardingState::get(opCtx);
    invariant(shardingState->enabled());
    ...
    auto cmdResponse = uassertStatusOK(selfShard->runCommandWithFixedRetryAttempts(
        ...
        ReadPreferenceSetting{ReadPreference::PrimaryOnly},
        "admin",
        BSON("forceDatabaseRefresh" << dbName),
        Seconds{30},
        Shard::RetryPolicy::kIdempotent));
        ...
        _threadPool.schedule([ this, dbName, callbackFn, isPrimary, currentTerm ]() noexcept {
            ...
            try {
                if (isPrimary) {
                    ...
                    _runSecondaryGetDatabase(context.opCtx(), dbName, callbackFn);
                }
            } catch (const DBException& ex) {
                callbackFn(context.opCtx(), ex.toStatus());
            }
...
void ShardServerCatalogCacheLoader::_runSecondaryGetDatabase(
    ...
    StringData dbName,
    stdx::function<void(OperationContext*, StatusWith<DatabaseType>)> callbackFn) {
    forcePrimaryRefreshDbVersionAndWaitForReplication(opCtx, dbName);

    // Read the local metadata.
    auto swDatabaseType =
        getPersistedDbVersion(opCtx, dbName);
    ...
    auto remoteRefreshCallbackFn = [this, dbName, maxLoaderVersion, termScheduled, callbackFn](
        ...
        if (swDatabaseType == ErrorCodes::NamespaceNotFound) {
            ...
            Status scheduleStatus = _ensureMajorityPrimaryAndScheduleDbTask(
                opCtx, dbName, dbTask{swDatabaseType, maxLoaderVersion, termScheduled});
            if (!scheduleStatus.isOK()) {
                callbackFn(opCtx, scheduleStatus);
                return;
            }
...
Status ShardServerCatalogCacheLoader::_ensureMajorityPrimaryAndScheduleDbTask(
    ...
    if (wasEmpty) {
        Status status = _threadPool.schedule([this, dbName]() { _runDbTasks(dbName); });
        if (!status.isOK()) {
            log() << "Cache loader failed to schedule persisted metadata update"
...
void ShardServerCatalogCacheLoader::DbTaskList::addTask(dbTask task) {
    ...
    if (task.dropped) {
        // version should be boost::none
        invariant(!bool(_tasks.back().maxVersion) && !bool(task.minVersion));
