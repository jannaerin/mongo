...
(function() {
    ...
    assert.writeOK(st.rs0.getPrimary().getDB('admin').system.version.insert(recoveryDoc));

    // Make sure test is setup correctly.
    var minOpTimeRecoveryDoc =
        st.rs0.getPrimary().getDB('admin').system.version.findOne({_id: 'minOpTimeRecovery'});
